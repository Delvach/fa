...
5. the current immediate queue is:
   - `0.6.64` is the active raw operator seam, keeping the `0.6.63` plugin UI/default-preset layout but moving the default-preset startup state into plugin data and leaving the startup apply armed until the attached player is actually ready
   - `0.6.65`: slideshow
...
1. direct-player raw asset:
   `F:\sim\vam\Custom\Assets\FrameAngel\Player\asset_dev_player.0.6.64.alpha.assetbundle`
2. raw plugin:
   `F:\sim\vam\Custom\Plugins\plugin_player_dev.0.6.64.alpha.dll`
3. deterministic live scene:
   `F:\sim\vam\Saves\scene\scene_dev_player.0.6.64.alpha.json`
4. release validation receipt:
   `C:\projects\fa\products\vam\assets\player\build\releases\0.6.64\foundation_release_validation.json`
