...
5. the current immediate queue is:
   - `0.6.55` is the active raw operator seam, restoring the 0.6.28 fixed normalized movie scrub rate inside the current scrub-session model after 0.6.54 recovered the broader joystick/input contract
   - `0.6.56`: slideshow
   - `0.6.57`: right-stick up/down volume without trigger
   - `0.6.58`: deferred gaze-enable polish for unloaded or cleared screens
...
1. direct-player raw asset:
   `F:\sim\vam\Custom\Assets\FrameAngel\Player\asset_dev_player.0.6.55.alpha.assetbundle`
2. raw plugin:
   `F:\sim\vam\Custom\Plugins\plugin_player_dev.0.6.55.alpha.dll`
3. deterministic live scene:
   `F:\sim\vam\Saves\scene\scene_dev_player.0.6.55.alpha.json`
4. release validation receipt:
   `C:\projects\fa\products\vam\assets\player\build\releases\0.6.55\foundation_release_validation.json`
