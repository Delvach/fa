...
5. the current immediate queue is:
   - `0.6.48` is the active raw operator test seam for image slider navigation and state
   - `0.6.49`: more aggressive scrub feedback while dragging
   - `0.6.50`: slideshow
   - `0.6.51`: right-stick up/down volume without trigger
   - `0.6.52`: deferred gaze-enable polish for unloaded or cleared screens
...
1. direct-player raw asset:
   `F:\sim\vam\Custom\Assets\FrameAngel\Player\dev_cua_player.0.6.48.assetbundle`
2. raw plugin:
   `F:\sim\vam\Custom\Plugins\dev_plugin_player.0.6.48.dll`
3. deterministic live scene:
   `F:\sim\vam\Saves\scene\demo3.0.6.48.json`
4. release validation receipt:
   `C:\projects\fa\products\vam\assets\player\build\releases\0.6.48\foundation_release_validation.json`
...
12. the current mixed-media policy is still explicit: mixed directories are valid playlists, image slider navigation is now an intentional `0.6.48` transport extension, and slideshow remains a separate later seam
