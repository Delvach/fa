...
5. the current immediate queue is:
   - `0.6.50` is the active raw operator test seam for more immediate scrub feedback while dragging
   - `0.6.51`: slideshow
   - `0.6.52`: right-stick up/down volume without trigger
   - `0.6.53`: deferred gaze-enable polish for unloaded or cleared screens
   - `0.6.54`: investigate image-only gaze hit feasibility if still wanted after the unloaded-screen gaze gate
...
1. direct-player raw asset:
   `F:\sim\vam\Custom\Assets\FrameAngel\Player\dev_cua_player.0.6.50.assetbundle`
2. raw plugin:
   `F:\sim\vam\Custom\Plugins\dev_plugin_player.0.6.50.dll`
3. deterministic live scene:
   `F:\sim\vam\Saves\scene\demo3.0.6.50.json`
4. release validation receipt:
   `C:\projects\fa\products\vam\assets\player\build\releases\0.6.50\foundation_release_validation.json`
...
12. the current mixed-media policy is still explicit: mixed directories are valid playlists, image slider navigation is an intentional transport extension, discovered numbered media now follow natural sequence when shuffle is off, and `0.6.50` is only a scrub-feedback/readback slice rather than a transport-model rewrite
