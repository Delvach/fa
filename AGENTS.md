...
5. the current immediate queue is:
   - `0.6.65` is the active raw operator seam, keeping the `0.6.64` player baseline but adding a minimal image-only slideshow timer plus plugin timing dropdowns without changing movie transport or image scrub semantics
   - `0.6.66`: right-stick up/down volume without trigger
...
1. direct-player raw asset:
   `F:\sim\vam\Custom\Assets\FrameAngel\Player\asset_dev_player.0.6.65.alpha.assetbundle`
2. raw plugin:
   `F:\sim\vam\Custom\Plugins\plugin_player_dev.0.6.65.alpha.dll`
3. deterministic live scene:
   `F:\sim\vam\Saves\scene\scene_dev_player.0.6.65.alpha.json`
4. release validation receipt:
   `C:\projects\fa\products\vam\assets\player\build\releases\0.6.65\foundation_release_validation.json`
...
12. the current mixed-media policy is still explicit: mixed directories are valid playlists, image slider navigation is an intentional transport extension, discovered numbered media follow natural sequence when shuffle is off, `0.6.59` restores shuffle defaults to off, `0.6.60` keeps shuffle toggle control parity aligned with that policy while tightening small AV drift correction, `0.6.61` adds a manual dev checkbox for the direct CUA `disableNavigation` capture bit without restoring the later button-5 fallback seam, `0.6.62` collapses the visible player readout into one operator-facing summary block without reopening the runtime transport model, `0.6.63` reorganizes the plugin UI and default-preset seam without changing transport behavior, `0.6.64` makes the default-preset startup seam plugin-data-backed and host-ready instead of relying on an early expiring startup probe, and `0.6.65` adds a minimal image-only slideshow timer plus configurable Video Scrub and Image Slideshow timing controls without touching movie seek or sync paths
