...
5. the current immediate queue is:
   - `0.6.69` is the active raw operator seam, keeping the `0.6.68` player baseline but adding minimal single-source spatial audio by anchoring the player AudioSource at the hosted CUA root while keeping the existing playback runtime and transport model intact
   - `0.6.70`: deferred gaze-enable polish for unloaded or cleared screens
   - `0.6.71`: favorites planning and implementation packeting remains separate from current transport polish
   - `0.6.72`: revisit the default-preset autoload seam after the current input slice settles
...
1. direct-player raw asset:
   `F:\sim\vam\Custom\Assets\FrameAngel\Player\asset_dev_player.0.6.69.alpha.assetbundle`
2. raw plugin:
   `F:\sim\vam\Custom\Plugins\plugin_player_dev.0.6.69.alpha.dll`
3. deterministic live scene:
   `F:\sim\vam\Saves\scene\scene_dev_player.0.6.69.alpha.json`
4. release validation receipt:
   `C:\projects\fa\products\vam\assets\player\build\releases\0.6.69\foundation_release_validation.json`
