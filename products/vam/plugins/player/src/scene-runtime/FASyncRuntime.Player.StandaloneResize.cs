...
public partial class FASyncRuntime : MVRScript
{
    ...
    private static readonly float AttachedHostedPlayerResizeDurationSeconds = 0.25f;
    ...
    private bool TryResizeAttachedHostedPlayerHostScale(
        StandalonePlayerRecord record,
        Atom hostAtom,
        float multiplier,
        ...
        out string errorMessage)
    {
        ...
        if (hostAtom == null)
        {
            errorMessage = "hosted player host atom not resolved";
            resultJson = BuildBrokerResult(false, errorMessage, "{}");
            return false;
        }
        ...
        if (multiplier <= 0f)
        {
            errorMessage = "player resize multiplier invalid";
            resultJson = BuildBrokerResult(false, errorMessage, "{}");
            return false;
        }
        ...
        {
            errorMessage = "hosted player host scale parameter not resolved";
            resultJson = BuildBrokerResult(false, errorMessage, "{}");
            return false;
        }

        float startScale = Mathf.Max(0.01f, scaleParam.val);
        float targetScale = Mathf.Clamp(startScale * multiplier, 0.01f, 100f);

        FAInnerPiecePlaneData plane;
        string planeError;
        if (TryResolveInnerPieceScreenPlane(record.instanceId, record.slotId, out plane, out planeError))
        {
            record.targetDisplayWidthMeters = Mathf.Max(0.05f, plane.widthMeters * multiplier);
            record.targetDisplayHeightMeters = Mathf.Max(0.05f, plane.heightMeters * multiplier);
        }

        record.resizeBehavior = "smooth";
        record.resizeAnchor = "";

        StopStandalonePlayerResize(record);
        if (AttachedHostedPlayerResizeDurationSeconds > 0.0001f)
        {
            record.resizeInFlight = true;
            record.resizeProgressNormalized = 0f;
            record.resizeStartedAt = Time.unscaledTime;
            record.resizeDurationSeconds = AttachedHostedPlayerResizeDurationSeconds;
            record.resizeCoroutine = StartCoroutine(
                RunStandalonePlayerHostScaleResize(
                    record,
                    scaleParam,
                    startScale,
                    targetScale,
                    AttachedHostedPlayerResizeDurationSeconds,
                    "Player.ResizeHostScale",
                    "",
                    ""));
        }
        else
        {
            ApplyStandalonePlayerHostScaleResizeStep(record, scaleParam, targetScale);
            record.resizeInFlight = false;
            record.resizeProgressNormalized = 1f;
            record.resizeStartedAt = 0f;
            record.resizeDurationSeconds = 0f;
        ...
        string payload = BuildStandalonePlayerSelectedStateJson("{\"playbackKey\":\"" + EscapeJsonString(record.playbackKey) + "\"}");
        resultJson = BuildBrokerResult(true, "player_host_scale_resize ok", payload);
        EmitRuntimeEvent(
            "player_host_scale_resize",
            "Player.ResizeHostScale",
            "ok",
            "",
            record.playbackKey,
    ...
    private IEnumerator RunStandalonePlayerHostScaleResize(
        StandalonePlayerRecord record,
        JSONStorableFloat scaleParam,
        float startScale,
        float targetScale,
        ...
        string messageId)
    {
        if (record == null || scaleParam == null)
            yield break;
        ...
        while (record != null && scaleParam != null)
        {
            float elapsed = Time.unscaledTime - startedAt;
            float t = durationSeconds <= 0.0001f ? 1f : Mathf.Clamp01(elapsed / durationSeconds);
            ApplyStandalonePlayerHostScaleResizeStep(
                record,
                scaleParam,
    ...
    private void ApplyStandalonePlayerHostScaleResizeStep(
        StandalonePlayerRecord record,
        JSONStorableFloat scaleParam,
        float targetScale)
    {
        ...
        scaleParam.val = Mathf.Clamp(targetScale, 0.01f, 100f);
        if (record != null)
